...
class Student(CertifiedCommunicatingParty):
    ...
    def __init__(self,
       ...
       """
        ...
        self.studentInfo = StudentInfo(matricola_casa, matricola_ospitante, nome, cognome, email_casa,
                                       email_ospitante, data_di_nascita, codice_corso_di_laurea,
                                       nome_corso_di_laurea, cfu_totali_conseguiti, media_voti)

    def introduce(self):
        self.studentInfo.introduce()

    def communicate(self, message: str) -> bytes:
        print(f"{self.studentInfo.nome} {self.studentInfo.cognome} sta inviando un messaggio criptato...")
